...
const tryTimeoutMsg = '请求每次重试的间隔时长 格式: 1h/1m/1s/1ms. 必须 >=1ms.';

export class RouteRetry extends React.Component<RouteRetryProps> {
  render() {
    return (
      <>
        <FormGroup label="添加超时重试设置" fieldId="retrySwitch">
          <Switch
            id="retrySwitch"
            label={' '}
            labelOff={' '}
        ...
        {this.props.isRetry && (
          <>
            <FormGroup label="重试次数" fieldId="attempts">
              <TextInput
              ...
              <FormHelperText>
                <HelperText>
                  <HelperTextItem>设定请求的重试次数</HelperTextItem>
                </HelperText>
              </FormHelperText>
            </FormGroup>
            <FormGroup label="重试间隔" fieldId="pre-try-timeout">
              <TextInput
